...
const severityConfig: Record<string, { color: string; bg: string; border: string }> = {
  Low: { color: 'text-zinc-400', bg: 'bg-zinc-500/10', border: 'border-zinc-500/20' },
...
export default function RulesPage() {
  const { rules, addRule } = useMockStore();
  const { push } = useToast();
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-6">
        <div>
          <h1 className="font-display text-3xl font-bold text-white tracking-tight">Rules Engine</h1>
          <p className="mt-2 text-sm text-zinc-400 max-w-xl">
            Define detection policies — thresholds, required fields, and cross-document consistency checks.
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-zinc-400 bg-zinc-900 px-3 py-1.5 rounded-full border border-white/5">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
        ...
        <section className="space-y-3">
              ...
              <div key={r.id} className="card-premium p-5 group hover:-translate-y-0.5 transition-all duration-200">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <div className="p-2 rounded-lg bg-white/5 text-zinc-400 group-hover:bg-indigo-500/10 group-hover:text-indigo-400 transition-colors shrink-0 mt-0.5">
                      <TypeIcon size={18} />
                    </div>
                    <div className="min-w-0">
                      <h3 className="text-sm font-semibold text-white group-hover:text-indigo-300 transition-colors">{r.name}</h3>
                      <p className="text-xs text-zinc-500 mt-1 font-mono">{r.config}</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/5">
                  <div className="flex items-center gap-4 text-xs text-zinc-500">
                    <span>Type: <span className="text-zinc-300 font-medium">{r.type}</span></span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-mono text-zinc-400 w-8 text-right">{r.weight}</span>
                  </div>
                </div>
              </div>
            );
          ...
          {rules.length === 0 && (
            <div className="card-premium py-16 text-center">
              <AlertTriangle className="mx-auto mb-3 text-zinc-600" size={32} />
              <p className="text-sm font-medium text-white">No rules configured</p>
              <p className="text-xs text-zinc-500 mt-1">Create your first detection rule to get started.</p>
            </div>
          )}
        </section>
        ...
        <section className="card-premium h-fit sticky top-24">
          <div className="p-6 border-b border-white/5 flex items-center gap-3">
            ...
            <div>
              <h3 className="font-display text-lg font-bold text-white">New Rule</h3>
              <p className="text-xs text-zinc-500">Add detection logic to the risk engine</p>
            </div>
          </div>
          <div className="p-6">
            <form className="space-y-5" onSubmit={onCreate}>
              <div>
                <label className="mb-2 block text-xs font-medium text-zinc-500 uppercase tracking-wide">Rule Name</label>
                <input
              ...
              <div>
                <label className="mb-2 block text-xs font-medium text-zinc-500 uppercase tracking-wide">Type</label>
              ...
              <div>
                <label className="mb-2 block text-xs font-medium text-zinc-500 uppercase tracking-wide">Configuration</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-2 block text-xs font-medium text-zinc-500 uppercase tracking-wide">Severity</label>
                  <div className="relative">
                    <select
                      className="input-field appearance-none cursor-pointer"
                      value={severity}
                ...
                <div>
                  <label className="mb-2 block text-xs font-medium text-zinc-500 uppercase tracking-wide">Weight (1–100)</label>
                  <input
                    type="number"
                    className="input-field"
                    value={weight}
